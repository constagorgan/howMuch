...
class GetEvent {
  ...
  public static function getEvents(){
    ...
    if($key && $name){
      ...
      $sql = "select events.id, events.name, events.hashtag, events.location, events.locationMagicKey, events.eventDate, events.description, events.creatorUser, events.duration, events.featured, events.private, events.isLocal, events.background, special_effects_map.special_effect_id AS 'specialEffect' from events LEFT JOIN special_effects_map ON special_effects_map.event_id = events.id WHERE id=?";
      $stmt = $link->prepare($sql);
